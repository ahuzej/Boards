    ...
    if (isStatusOk(status)) {
      try {
        ...
        req.payloadInfo = schemaErrorResponse(
          req.payloadInfo,
          err,
          "Error occured while trying to fetch thread data."
        );
      }
    ...
    if (isStatusOk(status)) {
      try {
        ...
        req.payloadInfo = schemaErrorResponse(
          req.payloadInfo,
          err,
          "Error while inserting new thread!"
        );
      }
 ...
 */
router.post(
  "/:threadId/comments/create",
  async function (req, res, next) {
    logger.info(
      "Entered threadsRouter.js post /threads/:threadId/comments/create"
    );
    ...
    if (isStatusOk(status)) {
      try {
        ...
        await data
          .populate("author", "username boards avatarUrl")
          .execPopulate();
        ...
        logger.info(
          `Added comment to thread id ${threadId}, comment ${JSON.stringify(
            data
          )}`
        );
      } catch (err) {
        req.payloadInfo = schemaErrorResponse(
          req.payloadInfo,
          err,
          "Error occured while adding new tasks!"
        );
      }
    ...
    logger.info(
      "Exiting threadsRouter.js post /threads/:threadId/comments/create"
    );
    next();
  },
  prepareResponse
 ...
 */
router.post(
  "/:threadId/changeThreadState",
  async function (req, res, next) {
    logger.info(
      "Entered threadsRouter.js post /threads/:threadId/changeThreadState"
    );
    ...
    if (isStatusOk(status)) {
      try {
        ...
        req.payloadInfo = schemaErrorResponse(
          req.payloadInfo,
          err,
          "An error has occured while updating!"
        );
      }
    ...
    logger.info(
      "Exiting threadsRouter.js post /threads/:threadId/changeThreadState"
    );
    next();
  },
  prepareResponse
    ...
    if (isStatusOk(status)) {
      try {
        ...
        if (currentRating) {
          ...
          logger.info(
            `User ${id} rating ${comment} (thread ${thread}) for the first time...`
          );
          // first time rating
